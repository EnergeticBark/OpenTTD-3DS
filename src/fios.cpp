...
#elif defined(N3DS)
 	/* The 3DS is already putting a PATHSEP (slash) at the end of its _fios_path's for some reason.
 	 * It wont save if theres 2 slashes, so we don't put the extra one. */
 	snprintf(buf, size, "%s%s%s", _fios_path, name, extension);
#else
	snprintf(buf, size, "%s" PATHSEP "%s%s", _fios_path, name, extension);
#endif
